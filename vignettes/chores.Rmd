...
- As of early 2025, Anthropic's **Claude Haiku 4.5** is a fast, cost-effective model for code assistance. If you want to use Claude, you'll need to register an API key at `https://console.anthropic.com/` to the environment variable `ANTHROPIC_API_KEY` and then set `options(.chores_chat = ellmer::chat_claude(model = "claude-haiku-4-5-20251001"))`.

* Regarding OpenAI's models, `chat_openai()` defaults to **GPT-4.1**, but you can use `model = "gpt-4.1-mini"` for a cheaper alternative; to use an OpenAI model, you'll need to set the options `options(.chores_chat = ellmer::chat_openai(model = "gpt-4.1"))` and register your OpenAI API key with the `OPENAI_API_KEY` environment variable.
