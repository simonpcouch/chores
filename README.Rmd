...
The cost of using helpers depends on 1) the length of the underlying prompt for a given helper and 2) the cost per token of the chosen model. Using the cli helper with Anthropic's Claude Haiku 4.5, for example, [costs something like](https://simonpcouch.github.io/chores/reference/cli_helper.html#cost) \$5 per 1,000 code refactorings, while using the testthat helper with OpenAI's GPT-4.1-mini would cost something like \$2 per 1,000 refactorings. Chore helpers using a locally-served LLM are "free" (in the usual sense of code execution, ignoring the cost of increased battery usage).
